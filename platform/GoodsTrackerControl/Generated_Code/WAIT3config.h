...
#ifndef __WAIT3_CONFIG_H
#define __WAIT3_CONFIG_H

#include "MCUC1.h" /* include library configuration */

#ifndef WAIT3_CONFIG_USE_CYCLE_COUNTER
  #define WAIT3_CONFIG_USE_CYCLE_COUNTER  (0 && (MCUC1_CONFIG_CORTEX_M>=3))
    /*!< 1: Use hardware cycle counter (if present, only on Cortex-M3 or higher), 0: not using hardware cycle counter */
#endif

#ifndef WAIT3_CONFIG_USE_RTOS_WAIT
  #define WAIT3_CONFIG_USE_RTOS_WAIT  (0)
    /*!< 1: Use RTOS wait if RTOS is present; 0: use normal busy waiting */
#endif

#if WAIT3_CONFIG_USE_CYCLE_COUNTER
  #include "KIN1.h" /* include Cortex utility functions */
#endif

#endif /* __WAIT3_CONFIG_H */
